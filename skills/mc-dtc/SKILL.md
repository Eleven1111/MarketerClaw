...
| 层级 | 工具 | 覆盖场景 | 目标 |
|------|------|---------|------|
| L1 自助 | FAQ / Help Center / 聊天机器人 | 物流查询、退换流程、常见问题 | 拦截 60%+ 的咨询 |
| L2 智能 | AI 客服（基于订单数据自动回复）| 订单状态、退款进度、个性化推荐 | 拦截 20%+ |
| L3 人工 | 真人客服（邮件/在线/电话）| 复杂问题、投诉、高价值客户 | 处理剩余 20% |

### 退货体验设计

| 要素 | 标准做法 | 超预期做法 |
|------|---------|-----------|
| 退货期限 | 30 天（美国行业标准）| 60-90 天（减少购买焦虑）|
| 退货流程 | 提供预付退货标签 | 一键在线申请 + 自动生成标签 |
| 退款速度 | 收到退货后 5-7 天 | 发起退货即退款（高信誉客户）|
| 退货原因收集 | 必填下拉选单 | 可选 + 自由填写（收集真实反馈）|
| 部分退款 | 不支持 | 商品有瑕疵时提供"留着 + 部分退款"选项 |

### T-A-S-C 危机沟通框架

当出现批量客诉/产品质量问题/物流延迟等危机时：

| 步骤 | 名称 | 动作 |
|------|------|------|
| T | Transparent（透明）| 第一时间公开问题，不隐瞒 |
| A | Acknowledge（承认）| 承认影响，表达歉意 |
| S | Solve（解决）| 给出具体解决方案 + 时间表 |
| C | Compensate（补偿）| 超出预期的补偿（折扣/赠品/延长会员）|

**模板：**
> 尊敬的客户，我们发现 [问题描述]，这给您带来了不便，我们深感抱歉。
> 我们已经 [解决措施]，预计 [时间] 内完全恢复。
> 作为补偿，我们为您提供 [具体补偿]。
> 如有任何问题，请随时联系 [联系方式]。
```
...
当品牌考虑从独立站扩张到其他渠道（Amazon / TikTok Shop / Etsy 等）时，用 MEURO 模型评估：
...
| 维度 | 权重 | 评分 (1-10) | 说明 |
|------|------|------------|------|
| **M** Market（市场容量）| 25% | | 该渠道在目标品类的 GMV 和增速 |
| **E** Economics（经济模型）| 25% | | 佣金/广告/物流成本 vs 客单价和毛利 |
| **U** Urgency（紧迫度）| 15% | | 竞品是否已入驻？先发优势窗口多大？ |
| **R** Resources（资源需求）| 20% | | 需要多少人力/系统/库存投入 |
| **O** Opportunity（独特机会）| 15% | | 该渠道是否有独立站不具备的能力（如 FBA 物流/平台流量）|

**MEURO 总分 = M×0.25 + E×0.25 + U×0.15 + R×0.20 + O×0.15**

| 总分 | 建议 |
|------|------|
| >7.0 | 立即启动 90 天测试 |
| 5.0-7.0 | 有潜力，但先解决资源或经济性问题 |
| <5.0 | 暂不进入，6 个月后重评 |
...
| 阶段 | 时间 | 核心任务 | 退出条件 |
|------|------|---------|---------|
| 设置期 | D1-D14 | 账号注册、Listing 上传、基础优化 | — |
| 测试期 | D15-D60 | 广告测试、定价测试、评价积累 | 月亏损 > 预算 150% |
| 评估期 | D61-D90 | 分析 ROI、规模化可行性、资源消耗 | ROI <0.5 或运营成本 > 收入 40% |

**评估通过：** 正式纳入渠道组合 → 更新 mc-dashboard 看板
**评估未通过：** 撤出或降级为"最低维护"状态

### 渠道蚕食检测

新渠道上线后，监控以下指标判断是否蚕食独立站：

| 指标 | 健康信号 | 蚕食信号 |
|------|---------|---------|
| 独立站收入 | 持平或增长 | 下降 >10% |
| 独立站自然流量 | 不变 | 下降（品牌词被平台截流）|
| 新渠道客户重叠度 | <30% | >50%（同一批人换了渠道买）|
| 总 GMV | 增长 | 持平（只是渠道转移）|

蚕食度 >30% 时，需要重新评估渠道定价和差异化策略。
```
...
由 mc-cmo 路由或 mc-orchestrate 编排时，slug 和执行流程以调用方为准，跳过本节。用户用 `/mc-dtc` 直接调用时，按以下步骤落盘，让文件、`.status.json` 与 WebUI 保持一致：
...
4. 初始化：`node "$SCRIPTS_DIR/setup.mjs" --slug "{slug}" --skill "mc-dtc" --step "dtc"`
5. 把完整产出（含交付卡）写入 `/tmp/mc-{slug}-dtc.md`，不在对话中输出正文。
6. 后处理：`node "$SCRIPTS_DIR/finalize.mjs" --slug "{slug}" --step "dtc" --file "dtc.md" --skill "mc-dtc" --input /tmp/mc-{slug}-dtc.md`，把输出原样返回用户，不添加任何内容。
7. 找不到脚本或没有 node 时不要中断：
   - 直接把产出写入 `campaigns/{slug}/dtc.md`；
   - `.status.json` 不存在时先写入 `{"campaignSlug":"{slug}","status":"running","steps":{},"review_loop":{"iteration":0,"max":3}}`，再把 `steps.dtc` 记为 `{"status":"done","file":"dtc.md"}`；
