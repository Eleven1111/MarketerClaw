...
由 mc-cmo 路由或 mc-orchestrate 编排时，slug 和执行流程以调用方为准，跳过本节。用户用 `/mc-aigc` 直接调用时，按以下步骤落盘，让文件、`.status.json` 与 WebUI 保持一致：
...
4. 初始化：`node "$SCRIPTS_DIR/setup.mjs" --slug "{slug}" --skill "mc-aigc" --step "aigc"`
5. 把完整产出（含交付卡）写入 `/tmp/mc-{slug}-aigc.md`，不在对话中输出正文。
6. 后处理：`node "$SCRIPTS_DIR/finalize.mjs" --slug "{slug}" --step "aigc" --file "aigc.md" --skill "mc-aigc" --input /tmp/mc-{slug}-aigc.md`，把输出原样返回用户，不添加任何内容。
7. 找不到脚本或没有 node 时不要中断：
   - 直接把产出写入 `campaigns/{slug}/aigc.md`；
   - `.status.json` 不存在时先写入 `{"campaignSlug":"{slug}","status":"running","steps":{},"review_loop":{"iteration":0,"max":3}}`，再把 `steps.aigc` 记为 `{"status":"done","file":"aigc.md"}`；
