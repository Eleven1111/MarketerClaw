...
这两条纪律和直接调用时的产出流程，都写在各技能交付规范之前的「通用纪律」区块里：原文只有一份（仓库 `templates/disciplines/`），由 `scripts/sync-disciplines.mjs` 按适用范围嵌入各技能，测试保证区块与原文一致。这样技能单独安装、或被 `/mc-xxx` 直接调用时，纪律同样生效。

路由执行时以被执行技能内的区块为准；mc-cmo 自己给出判断和建议时同样遵守：行动建议附 ICE，关键数字标注 `[实测·来源]` / `[估算·来源]` / `[推断]`，`[推断]` 依据的建议 Confidence 不超过 5。
